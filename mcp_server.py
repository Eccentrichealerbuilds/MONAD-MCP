...
def main(port: int, transport: str, chain_id: list[int]):
    ...
    @mcp.tool()
    async def get_transaction_history_tool(address: str) ->List[Dict[str, Any]
        ]:
        """
        Fetches the MOST RECENT page (default ~50 transactions, sorted descending) of wallet
        transaction history from Thirdweb Insight API for Monad Testnet.
        Requires THIRDWEB_CLIENT_ID environment variable.

        Args:
            address (str): Wallet address (e.g., "0xabc...").

        Returns:
            list: A list of recent transaction dictionaries. Raises error on failure.
        """
        print(
            f'TOOL WRAPPER: Called get_transaction_history_tool for {address}',
            file=sys.stderr)
        return await insight_service.get_transaction_history(address=address)
